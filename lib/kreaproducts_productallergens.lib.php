...
function productallergensPrepareHead($object)
{
	...
	$head[$h][0] = dol_buildpath("/kreaproducts/productallergens_card.php", 1).'?id='.$object->id;
	$head[$h][1] = $langs->trans("ProductAllergens");
	$head[$h][2] = 'card';
	$h++;

	if ($showtabofpagecontact) {
		$head[$h][0] = dol_buildpath("/kreaproducts/productallergens_contact.php", 1).'?id='.$object->id;
	...
	if ($showtabofpagenote) {
		if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
			...
			$head[$h][0] = dol_buildpath('/kreaproducts/productallergens_note.php', 1).'?id='.$object->id;
	...
	if ($showtabofpagedocument) {
		...
		$upload_dir = $conf->kreaproducts->dir_output."/productallergens/".dol_sanitizeFileName($object->ref);
		$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
		$nbLinks = Link::count($db, $object->element, $object->id);
		$head[$h][0] = dol_buildpath("/kreaproducts/productallergens_document.php", 1).'?id='.$object->id;
	...
	if ($showtabofpageagenda) {
		$head[$h][0] = dol_buildpath("/kreaproducts/productallergens_agenda.php", 1).'?id='.$object->id;
