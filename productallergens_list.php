...
/**
 *   	\file       productallergens_list.php
 *		\ingroup    kreaproducts
 *		\brief      List page for productallergens
 */
...
require_once __DIR__.'/class/productallergens.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('kreaproducts', 'productallergens', 'write');
	$permissiontodelete = $user->hasRight('kreaproducts', 'productallergens', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'ProductAllergens';
	$objectlabel = 'ProductAllergens';
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/kreaproducts/productallergens_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/kreaproducts/productallergens_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendProductAllergensRef";
$modelmail = "productallergens";
$objecttmp = new ProductAllergens($db);
