...
/**
 * \file        class/productallergens.class.php
 * \ingroup     kreaproducts
 * \brief       This file is a CRUD class file for ProductAllergens (Create/Read/Update/Delete)
 */ 
 ...
 */
class ProductAllergens extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'productallergens';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'kreaproducts_productallergens';

	/**
	 * @var string 	If permission must be checkec with hasRight('kreaproducts', 'read') and not hasright('mymodyle', 'productallergens', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'kreaproducts';

	/**
	 * @var string 	String with name of icon for productallergens. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'productallergens@kreaproducts' if picto is file 'img/object_productallergens.png'.
	 */
	...
	public $fields = array(
		...
		"traces" => array("type" => "integer", "label" => "Traces", "enabled" => "1", 'position' => 53, 'notnull' => 0, "visible" => "-2",),
	);
	public $rowid;
	...
	//  */
	// public $table_element_line = 'kreaproducts_productallergensline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_productallergens';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'ProductAllergensline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'ProductAllergens', 'fk_element'=>'fk_productallergens'));
	...
	//  */
	// protected $childtablesoncascade = array('kreaproducts_productallergensdet');

	// /**
	//  * @var ProductAllergensLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('kreaproducts', 'productallergens', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts', 'productallergens', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts', 'productallergens_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE ' . MAIN_DB_PREFIX . "ecm_files set filename = CONCAT('" . $this->db->escape($this->newref) . "', SUBSTR(filename, " . (strlen($this->ref) + 1) . ")), filepath = 'productallergens/" . $this->db->escape($this->newref) . "'";
				$sql .= " WHERE filename LIKE '" . $this->db->escape($this->ref) . "%' AND filepath = 'productallergens/" . $this->db->escape($this->ref) . "' and entity = " . $conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE ' . MAIN_DB_PREFIX . "ecm_files set filepath = 'productallergens/" . $this->db->escape($this->newref) . "'";
				$sql .= " WHERE filepath = 'productallergens/" . $this->db->escape($this->ref) . "' and entity = " . $conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->kreaproducts->dir_output . '/productallergens/' . $oldref;
				$dirdest = $conf->kreaproducts->dir_output . '/productallergens/' . $newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->kreaproducts->dir_output . '/productallergens/' . $newref, 'files', 1, '^' . preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowProductAllergens")];
		}
		$datas['picto'] = img_picto('', $this->picto) . ' <u>' . $langs->trans("ProductAllergens") . '</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowProductAllergens");
	...
	public function getLinesArray()
	{
		...
		$objectline = new ProductAllergensLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_productallergens:=:' . ((int) $this->id) . ')');
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('KREAPRODUCTS_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefProductAllergens $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_productallergens';
...
/**
 * Class ProductAllergensLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ProductAllergensLine extends CommonObjectLine
{
	// To complete with content of an object ProductAllergensLine
	// We should have a field rowid, fk_productallergens and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';        // Example: '' or 'productallergens'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';    // Example: '' or 'fk_productallergens'
