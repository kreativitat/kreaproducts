...
class Nutritional extends CommonObject
{
	...
	/**
	 * @var string 	If permission must be checkec with hasRight('kreaproducts', 'read') and not hasright('mymodyle', 'nutritional', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'kreaproducts';

	/**
	 * @var string 	String with name of icon for nutritional. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'nutritional@kreaproducts' if picto is file 'img/object_nutritional.png'.
	 */
	...
	public $fields = array(
		...
		"fk_product" => array("type" => "integer", "label" => "fk_product", "enabled" => "1", 'position' => 60, 'notnull' => 1, "visible" => "-2", "index" => "1",),
		"energy_kcal" => array("type" => "double(28,4)", "label" => "Energy (kcal)", "enabled" => "1", 'position' => 61, 'notnull' => 0, "visible" => "1",),
		"energy_kj" => array("type" => "double(28,4)", "label" => "Energy (kj)", "enabled" => "1", 'position' => 62, 'notnull' => 0, "visible" => "1",),
		"fat" => array("type" => "double(28,4)", "label" => "Fat", "enabled" => "1", 'position' => 63, 'notnull' => 0, "visible" => "1",),
		"saturates" => array("type" => "double(28,4)", "label" => "Saturates", "enabled" => "1", 'position' => 64, 'notnull' => 0, "visible" => "1",),
		"carbohydrates" => array("type" => "double(28,4)", "label" => "Carbohydrates", "enabled" => "1", 'position' => 65, 'notnull' => 0, "visible" => "1",),
		"sugars" => array("type" => "double(28,4)", "label" => "Sugars", "enabled" => "1", 'position' => 66, 'notnull' => 0, "visible" => "1",),
		"protein" => array("type" => "double(28,4)", "label" => "Protein", "enabled" => "1", 'position' => 67, 'notnull' => 0, "visible" => "1",),
		"salt" => array("type" => "double(28,4)", "label" => "Salt", "enabled" => "1", 'position' => 68, 'notnull' => 0, "visible" => "1",),
		"fiber" => array("type" => "double(28,4)", "label" => "Fiber", "enabled" => "1", 'position' => 69, 'notnull' => 0, "visible" => "1",),
	);
	public $rowid;
	...
	//  */
	// public $table_element_line = 'kreaproducts_nutritionalline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_nutritional';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Nutritionalline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Nutritional', 'fk_element'=>'fk_nutritional'));
	...
	//  */
	// protected $childtablesoncascade = array('kreaproducts_nutritionaldet');

	// /**
	//  * @var NutritionalLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('kreaproducts', 'nutritional', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts', 'nutritional', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts', 'nutritional_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE ' . MAIN_DB_PREFIX . "ecm_files set filename = CONCAT('" . $this->db->escape($this->newref) . "', SUBSTR(filename, " . (strlen($this->ref) + 1) . ")), filepath = 'nutritional/" . $this->db->escape($this->newref) . "'";
				$sql .= " WHERE filename LIKE '" . $this->db->escape($this->ref) . "%' AND filepath = 'nutritional/" . $this->db->escape($this->ref) . "' and entity = " . $conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE ' . MAIN_DB_PREFIX . "ecm_files set filepath = 'nutritional/" . $this->db->escape($this->newref) . "'";
				$sql .= " WHERE filepath = 'nutritional/" . $this->db->escape($this->ref) . "' and entity = " . $conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->kreaproducts->dir_output . '/nutritional/' . $oldref;
				$dirdest = $conf->kreaproducts->dir_output . '/nutritional/' . $newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->kreaproducts->dir_output . '/nutritional/' . $newref, 'files', 1, '^' . preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts','kreaproducts_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'KREAPRODUCTS_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'KREAPRODUCTS_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'KREAPRODUCTS_MYOBJECT_REOPEN');
	}
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('KREAPRODUCTS_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefNutritional $obj';
...
/**
 * Class NutritionalLine. You can also remove this and generate a CRUD class for lines objects.
 */
class NutritionalLine extends CommonObjectLine
{
	// To complete with content of an object NutritionalLine
	// We should have a field rowid, fk_nutritional and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'nutritional'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_nutritional'
