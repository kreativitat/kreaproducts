 ...
 * \brief       This file is a CRUD class file for Allergens (Create/Read/Update/Delete)
 */
 ...
 * Class for Allergens
 */
class Allergens extends CommonObject
{
	/**
	 ...
	 */
	public $table_element = 'kreaproducts_allergens';

	/**
	 * @var string 	If permission must be checkec with hasRight('kreaproducts', 'read') and not hasright('mymodyle', 'allergens', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'kreaproducts';

	/**
	 * @var string 	String with name of icon for allergens. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'allergens@kreaproducts' if picto is file 'img/object_allergens.png'.
	 */
	...
	//  */
	// public $table_element_line = 'kreaproducts_allergensline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_allergens';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Allergensline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Allergens', 'fk_element'=>'fk_allergens'));
	...
	//  */
	// protected $childtablesoncascade = array('kreaproducts_allergensdet');

	// /**
	//  * @var AllergensLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('kreaproducts', 'allergens', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts', 'allergens', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('kreaproducts', 'allergens_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'allergens/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'allergens/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'allergens/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'allergens/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->kreaproducts->dir_output.'/allergens/'.$oldref;
				$dirdest = $conf->kreaproducts->dir_output.'/allergens/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->kreaproducts->dir_output.'/allergens/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowAllergens")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Allergens").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/kreaproducts/allergens_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowAllergens");
	...
	public function getLinesArray()
	{
		...
		$objectline = new AllergensLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_allergens:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('KREAPRODUCTS_MYOBJECT_ADDON')) {
			$conf->global->KREAPRODUCTS_MYOBJECT_ADDON = 'mod_allergens_standard';
		}
		...
		if (getDolGlobalString('KREAPRODUCTS_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefAllergens $obj';
...
/**
 * Class AllergensLine. You can also remove this and generate a CRUD class for lines objects.
 */
class AllergensLine extends CommonObjectLine
{
	// To complete with content of an object AllergensLine
	// We should have a field rowid, fk_allergens and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'allergens'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_allergens'
