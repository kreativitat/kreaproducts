...
$customerTimestamp = $businessDay->resolveCustomerInvoiceValueTimestamp('2026-07-11', $timezone, '06:00');
assertSameValue('2026-07-12 06:00:00', (new DateTimeImmutable('@'.$customerTimestamp))->setTimezone($timezone)->format('Y-m-d H:i:s'), 'Customer invoice business-close normalization failed.');
...
assertSameValue(true, strpos((string) $moduleSource, "\$this->version = '4.5.12'") !== false, 'The module descriptor must use the audited release version.');
...
assertSameValue('4.5.12', $mobilePackage['version'] ?? '', 'The mobile package version must match the module release.');
assertSameValue('4.5.12', $mobilePackageLock['version'] ?? '', 'The mobile lockfile version must match the module release.');
assertSameValue('4.5.12', $mobilePackageLock['packages']['']['version'] ?? '', 'The mobile lockfile root package must match the module release.');
