...
/**
 *   	\file       allergens_list.php
 *		\ingroup    kreaproducts
 *		\brief      List page for allergens
 */
...
require_once __DIR__.'/class/allergens.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('kreaproducts', 'allergens', 'write');
	$permissiontodelete = $user->hasRight('kreaproducts', 'allergens', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Allergens';
	$objectlabel = 'Allergens';
...
$title = $langs->trans("Allergenss");
//$help_url = "EN:Module_Allergens|FR:Module_Allergens_FR|ES:Módulo_Allergens";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/kreaproducts/allergens_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/kreaproducts/allergens_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAllergensRef";
$modelmail = "allergens";
$objecttmp = new Allergens($db);
