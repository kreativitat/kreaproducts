...
/**
 *   	\file       nutritional_list.php
 *		\ingroup    kreaproducts
 *		\brief      List page for nutritional
 */
...
require_once __DIR__.'/class/nutritional.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('kreaproducts', 'nutritional', 'read');
	$permissiontoadd = $user->hasRight('kreaproducts', 'nutritional', 'write');
	$permissiontodelete = $user->hasRight('kreaproducts', 'nutritional', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Nutritional';
	$objectlabel = 'Nutritional';
	$uploaddir = $conf->kreaproducts->dir_output;
...
$title = $langs->trans("Nutritionals");
//$help_url = "EN:Module_Nutritional|FR:Module_Nutritional_FR|ES:Módulo_Nutritional";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/kreaproducts/nutritional_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-kreaproducts page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/kreaproducts/nutritional_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendNutritionalRef";
$modelmail = "nutritional";
$objecttmp = new Nutritional($db);
