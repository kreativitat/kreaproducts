...
foreach ($defaultchecked as $fieldkey) {
...
foreach ($forceunchecked as $fieldkey) {
	...
	if (!empty($htmlofselectarray)) {
		$pattern = '/(<input\\b[^>]*\\bvalue="' . preg_quote($fieldkey, '/') . '"[^>]*?)\\schecked="checked"/';
		$htmlofselectarray = preg_replace($pattern, '$1', $htmlofselectarray);
	}
}
