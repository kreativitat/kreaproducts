...
END;
			...
			if ($action != 'add_price' && $action != 'update_price') {
				...
				if (empty($reshook)) {
					if ($usercancreate) {
						print '<a class="butAction" href="' . DOL_URL_ROOT . '/product/price_suppliers.php?id=' . $object->id . '&action=create_price&token=' . newToken() . '">';
			...
			if ($user->hasRight("fournisseur", "read")) { // Duplicate ? this check is already in the head of this file
				...
				if ($user->rights->degema->dglistagemcompras->read) { // Não apagar
...
LIMIT 10";
					...
					if (is_array($result_array)) {
						foreach ($result_array as $productfourn) {
							print '<tr class="oddeven">';

							// Data
							print '<td>' . date('d/m/Y H:i:s', strtotime($productfourn['DataModificacao'])) . '</td>';

							// Supplier
							print '<td class="tdoverflowmax150">' . $productfourn['Fornecedor'] . '</td>';

							// SKU
							print '<td class="tdoverflowmax150">' . $productfourn['VendorSKU'] . '</td>';

							// Preço
							print '<td style="text-align:right">' . $productfourn['Preco'] . '</td>';
							print '</tr>';
						}
					} else {
						dol_print_error($db);
					...
					if (is_array($result_array)) {
						foreach ($result_array as $productfourn) {
							...
							// Quantidade
							print '<td class="right">' . number_format($productfourn['Quantidade'], 3) . '</td>';
							// IVA
							print '<td class="right">' . price2num($productfourn['IVA']) . '%' . '</td>';
							// Preço
							print '<td class="right">' . price($productfourn['Preco'] / $productfourn['Quantidade']) . '</td>';
